      ...
      <div class="container">
           ...
           foreach($result as $row)
                          {
                            $count++;
                            $tmpIP=$row['ADD_IP'];
                            $ProdName=$Produit1C->recupererProduitNom($row['id_produit']);
                            foreach($ProdName as $prod)
                            $PName=$prod['nom_produit'];
                            $ProdImg=$Produit1C->recupererProduitimgPath($row['id_produit']);
                             foreach($ProdImg as $prod)
                            $PImg=$prod['imgpath_produit'];
                             $ProdDesc=$Produit1C->recupererProduit($row['id_produit']);
                             foreach($ProdDesc as $prod)
                            $PPrix=$row['Prix']/$row['Qty'];
                          ?>
                  <tr>
                    <td class="product-thumbnail">
                      <img src="<?php echo $PImg; ?>" alt="Image" class="img-fluid">
                    </td>
                    <td class="product-name">
                      <h2 class="h5 text-black"><?php echo $PName;?></h2>
                    </td>
                    <td><?php echo $PPrix; ?></td>
                    <td><?php echo $row['Qty']; ?></td>
                    <td><?php echo $row['Prix']; ?></td>
                    <td>
                        <?php echo'<div class="position-center-center">
                                    <form method="POST" action="deleteCosCart.php" >
                                    <button type="submit" class="btni btn-dark" style="margin-bottom: 2px;" >X</button>
                                    
                                    <input type="hidden" value="'.$row['ADD_IP'].'" name="deleteIP">
                  <input type="hidden" value="'.$row['id_produit'].'" name="deletePID"> 
                  </form> 
                  </br>
                  <form method="POST" action="pay-updatecart.php">
                                    <button type="submit" class="btni btn-dark" >U</button>
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <form method="POST" action="pay-checkout.php">
              <div class="col-md-6">
            <input type="submit" name="update" class="btn btn-dark" value="PASSER LA COMMANDE" >
             </div>
            ...
            <div class="col-md-6">
            <input type="submit" name="update" class="btn btn-dark" value="CONTINUER VOS ACHATS" >
             </div>
            </form>
